...
def update(request):
    if request.method=='POST' and 'id' in request.POST:
        # item=Restaurant.objects.get(pk=request.POST.get('id')) #id 값에 해당하는 아이템을 불러옴
        item=get_object_or_404(Restaurant, pk=request.POST.get('id'))
        password = request.POST.get('password','')
        form=UpdateRestaurantForm(request.POST, instance=item) #데이터베이스에서 불러온 데이터를 사용
...
def review_create(request, restaurant_id):
    if request.method=='POST':
        ...
        if form.is_valid():
            new_item=form.save()
        return redirect('restaurant-detail', id=restaurant_id)  #redirect shortcut을 사용하여 경로를 전부 넣지 않아도 됨

    item=get_object_or_404(Restaurant, pk=restaurant_id)
    form=ReviewForm(initial={'restaurant':item})    #사용자에게 입력받기 전에 사전에 입력해야하는 값 ex) restaurant
...
def review_list(request):
    reviews = Review.objects.all().select_related().order_by('-created_at') #한 번의 join을 통해 가져온 정보로 query를 한 번만 호출하게 됨
